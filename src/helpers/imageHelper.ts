...
export const getEntityImagePath = (entityName: string, entityType: EntityType): string => {
  const slug = slugify(entityName);
  return `/uploads/${entityType}/${slug}.png`;
};

export const getEntityImage = (entityName: string, entityType: EntityType): Promise<string> => {
